...
public class GameHUD3D : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        DrawRect(new Rect(8, 6, 290, 112), new Color(0f, 0f, 0f, 0.45f));
        GUI.Label(new Rect(12, 10, 500, 30), $"Kills: {gm.Kills}", _style);
        GUI.Label(new Rect(12, 34, 500, 30), $"Monsters left: {gm.EnemiesRemaining}", _style);
        ...
        GUI.Label(new Rect(12, Screen.height - 36, 900, 30), "WASD - move   Mouse - aim   LMB/Space - fire   R - restart", _style);
        if (gm.PortalSpawned)
        {
            DrawRect(new Rect(8, 116, 560, 30), new Color(0f, 0.16f, 0f, 0.48f));
            GUI.Label(new Rect(12, 116, 900, 30), "Green portal appeared! Reach it to claim your prize.", _style);
        }

        if (!gm.GameOver) return;
        string text = gm.Win ? "YOU WIN (R to restart)" : "GAME OVER (R to restart)";
    ...
    void DrawStartPanel(GameManager3D gm)
    {
        ...
        GUI.Label(new Rect(x + 70f, y + 150f, panelW - 140f, 100f),
            "You have only 20 bullets to survive the monster wave.", _menuTextStyle);
        GUI.Label(new Rect(x + 70f, y + 236f, panelW - 140f, 100f),
            "Kill all monsters, then enter the green portal to claim your prize.", _menuTextStyle);
